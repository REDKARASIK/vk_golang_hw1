...
Так же к сервису может подключиться 2 (и более) модуля статистики с разными интервалами получения статистики (например, каждые 2, 3 и 5 секунд) и она будет асинхронно отправляться по каждому интерфейсу.
...
Как сгенерить код из proto-файла (способ 1, Новый):
...
Как сгенерить код из proto-файла (способ 2. Старый, deprecated):
- Ставим тулзу для генерации - protoc (это можно нагуглить)
